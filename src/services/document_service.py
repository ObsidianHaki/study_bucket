...
def validate_file_exists_in_collection(file_name: str) -> bool:
    """Check both registry and GridFS to prevent duplicates."""
    in_registry = registry.find_one({"filename": file_name}) is not None
    in_gridfs = MONGO_CLIENT["knowledge_pool.files"].find_one({"filename": file_name}) is not None
    return in_registry or in_gridfs
...
def _store_chunks(chunks: list[dict], filename: str):
    ...
    # Persist full document data in MongoDB FIRST (so file shows in library immediately)
    registry.insert_one({
        "filename": filename,
    ...
    logger.info("Registry saved | filename='%s' | chunks=%d", filename, len(chunks))
...
def save_document(file: UploadFile, background_tasks: BackgroundTasks):
    ...
    gridfs_bucket.upload_from_stream(filename, pdf_bytes, metadata={"content_type": "application/pdf"})
    logger.info("PDF stored in GridFS | filename='%s' | size=%d bytes", filename, len(pdf_bytes))
...
def reindex_from_mongo() -> dict:
    ...
    docs = list(registry.find({"chunks": {"$exists": True}}))
    total_chunks = 0
    ...
    for doc in docs:
        ...
        logger.info("Re-indexed | filename='%s' | chunks=%d", doc["filename"], len(chunks))
